...
type IncomeStatementAssignmentStatus =
  | "mapped_to_model_row"
  | "grouped_into_model_row"
  | "subtotal_or_total_excluded"
  | "explicitly_excluded_with_reason";

type PrimaryIncomeStatementAssignmentLedgerRow = {
  fiscalPeriod: string;
  sourceFilingAccession: string;
  sourceStatement: string;
  sourceLineItemLabel: string;
  ...
  sourceXbrlTag: string;
  assignedModelRow: string;
  assignmentStatus: IncomeStatementAssignmentStatus;
  classificationReason: string;
  llmUsed: boolean;
  validationStatus: string;
  sourceSection: FinancialStatementSection;
...
type PrimaryIncomeStatementAssignment = {
  modelRow: string | null;
  status: IncomeStatementAssignmentStatus;
  reason: string;
};
...
const INCOME_STATEMENT_ASSIGNMENT_LEDGER_SHEET = "Income Stmt Assignment Ledger";
const SEGMENT_ANALYSIS_ASSIGNMENT_LEDGER_SHEET = "Segment Assignment Ledger";
...
const CURRENT_OPERATING_LEASE_LIABILITY_CONCEPTS = [
  "OperatingLeaseLiabilityCurrent",
  "OperatingLeaseLiabilitiesCurrent",
  "LesseeOperatingLeaseLiabilityCurrent"
];
...
function sourceIsSeparatelyModeledOperatingExpense(source: FactSource) {
  ...
  if (sourceLooksLikeSellingGeneralAdministrativeExpense(source)) return true;
...
function sourceLooksLikeSellingGeneralAdministrativeExpense(source: FactSource) {
  ...
  return (
    ...
    /\bsales and marketing\b|\bselling and marketing\b|\bmarketing expense\b|\badvertising\b|\bpromotion(?:al)?\b|\bsales expense\b|\bselling expense\b/.test(text)
  );
}
...
function statementRowDurationPeriodType(row: SecFilingStatementStructure["rows"][number]): Exclude<FactSource["periodType"], "instant"> {
  if (row.period.periodType === "instant") return "annual";
  const start = row.period.start;
  const end = row.period.end ?? row.reportingPeriod;
  if (!start || !end) return "quarterly";
  ...
  if (days < 330) return "year_to_date";
  return "annual";
}
...
function classifiedSourceForModelRow(period: string, ctx: ResolveContext, source: FactSource, modelRow: string): FactSource | null | undefined {
  ...
  if (classificationRejectedOnlyBecauseResidualBucketIsGeneric(classification, modelRow)) {
    return attachLineItemClassification(source, classification, sourceSectionLabelForClassification(source, classification));
  }
...
function classificationRejectedOnlyBecauseResidualBucketIsGeneric(classification: FinancialLineItemClassification, modelRow: string) {
  if (classification.mapping_passed_validation) return false;
  if (!modelRowsMatch(classification.recommended_model_row, modelRow)) return false;
  if (!isReusableResidualBucketModelRow(modelRow)) return false;
  return /other buckets are allowed only when no better dedicated model row exists/i.test(classification.reason);
}

function isReusableResidualBucketModelRow(modelRow: string) {
  return (
...
function factSourceFromStatementRow(row: SecFilingStatementStructure["rows"][number], period: string): FactSource | null {
  ...
  return {
    ...
    periodKey: period,
    periodType: row.period.periodType === "instant" ? "instant" : statementRowDurationPeriodType(row),
    reportDate: row.reportingPeriod
  };
}
...
function primaryRowInNonCurrentAssetSection(row: PrimaryBalanceSheetRow, source: FactSource) {
  const parent = primaryRowParentConcept(row);
  const category = reportedLineItemCategory(source);
  if (category === "current_liabilities" || category === "non_current_liabilities" || category === "total_liabilities" || category === "equity") return false;
  if (parent === "Assets" && !primaryRowInCurrentAssetSection(row, source) && !isPrimaryBalanceSheetSubtotalSource(source)) return true;
  if (row.currentNonCurrentSection === "non_current") return category === "non_current_assets";
  return category === "non_current_assets";
}
...
function sourceLooksLikeOtherCurrentLiability(source: FactSource) {
  ...
  return sourceTextMatches(
    source,
    /\bother\b.*\bcurrent liabilit|\bliabilities?\b.*\bheld for sale\b|\bheld for sale\b.*\bliabilities?\b|\bdeferred (revenue|income)(?!\s+tax)\b|\bunearned revenue\b|\bcontract liabilit|\bcustomer advances?\b|\btaxes payable\b|\bother insurance liabilit|\bcurrent portion\b.*\blease liabilit/
  );
}
...
function sourceLooksLikeNonTradeReceivable(source: FactSource) {
  return sourceTextMatches(source, /\bvendor\b.*\bnon[-\s]?trade receivables?\b|\bnon[-\s]?trade receivables?\b|\bother receivables?\b|\btax receivables?\b/);
}
...
function primaryAccruedLiabilitySources(period: string, ctx: ResolveContext) {
  const sources = primaryBalanceSheetComponentSources(period, ctx, (source, row) => {
    ...
    return (
      ...
      !sourceTextMatches(source, /\bother insurance liabilit|\bdeferred (revenue|income)\b|\bunearned revenue\b|\bcontract liabilit|\bcustomer advances?\b/)
    );
  });
...
function primaryOtherCurrentLiabilitySources(period: string, ctx: ResolveContext) {
  ...
  return withAcceptedOtherCurrentLiabilityClassifications(period, ctx, sources);
}
...
function primaryStatementTotalRevenueOrder(statement: SecFilingStatementStructure) {
  return firstPrimaryStatementRowOrder(statement, (row) => primaryIncomeStatementRowLooksLikeTotalRevenue(row));
}

function primaryIncomeStatementRowLooksLikeTotalRevenue(row: SecFilingStatementStructure["rows"][number]) {
  const label = cleanLineItemLabel(row.rowLabel).toLowerCase();
  const concept = row.xbrlConcept ?? "";
  if (/\b(total|net)\b.*\brevenues?\b|\brevenues?\b.*\b(total|net)\b|\btotal net sales\b/.test(label)) return true;
  if (concept === "Revenues" && /\brevenues?\b/.test(label) && !/\bcost|expense/.test(label)) return true;
  return false;
}
...
function primaryIncomeStatementRowIsInRevenueSection(statement: SecFilingStatementStructure, row: SecFilingStatementStructure["rows"][number]) {
  ...
  const parentText = `${row.parentSubtotal?.label ?? ""} ${row.parentSubtotal?.concept ?? ""}`.toLowerCase();
  if (/\b(revenues?|sales)\b/.test(parentText) && !/\bcost|expense/.test(parentText)) return true;

  const totalRevenueOrder = primaryStatementTotalRevenueOrder(statement);
  if (totalRevenueOrder === null || row.rowOrder > totalRevenueOrder) return false;

  const rowText = `${row.rowLabel} ${row.xbrlConcept ?? ""}`.toLowerCase();
  if (/\bcost|expense|operating income|gross profit|gross margin|income before|pre[-\s]?tax|net income/.test(rowText)) return false;
  return true;
}
...
function resolveInterestExpense(period: string, ctx: ResolveContext): ResolvedValue {
  ...
  const primaryStatementSources = primaryStatementInterestExpenseSources(period, ctx);
  if (primaryStatementSources.length) {
    return {
      value: primaryStatementSources.reduce((total, source) => total + expenseAsModelReduction(source.value), 0),
...
function sourceHasStandalonePrimaryIncomeStatementInterestIncomeForResolver(period: string, ctx: ResolveContext, source: FactSource) {
  if (primaryIncomeStatementStructuresAvailable(ctx)) return sourceHasStandalonePrimaryIncomeStatementInterestIncomeComponentLine(period, ctx, source);
  return sourceHasStandalonePrimaryIncomeStatementInterestIncomeLine(period, ctx, source);
}

function sourceHasStandalonePrimaryIncomeStatementInterestExpenseForResolver(period: string, ctx: ResolveContext, source: FactSource) {
  if (primaryIncomeStatementStructuresAvailable(ctx)) return sourceHasStandalonePrimaryIncomeStatementInterestExpenseComponentLine(period, ctx, source);
  return sourceHasStandalonePrimaryIncomeStatementInterestExpenseLine(period, ctx, source);
}

function primaryIncomeStatementStructuresAvailable(ctx: ResolveContext) {
  return Boolean(ctx.filingPackageStatements?.some(isPrimaryIncomeStatementStructure));
}
...
function sourceHasStandalonePrimaryIncomeStatementInterestIncomeComponentLine(period: string, ctx: ResolveContext, source: FactSource) {
  const rows = primaryIncomeStatementRowsForSource(period, ctx, source);
...
function sourceHasStandalonePrimaryIncomeStatementInterestExpenseComponentLine(period: string, ctx: ResolveContext, source: FactSource) {
  const rows = primaryIncomeStatementRowsForSource(period, ctx, source);
  if (!rows.length) return false;
  return rows.some(({ statement, row }) => {
    const text = `${row.rowLabel} ${row.xbrlConcept ?? ""}`.replace(/([a-z])([A-Z])/g, "$1 $2").toLowerCase();
...
function otherIncomeExpenseLineShouldBeSplit(period: string, ctx: ResolveContext, source: FactSource) {
  ...
  if (!isCombinedBelowOperatingSummaryLine(source)) return false;
...
function isCombinedBelowOperatingSummaryLine(source: FactSource) {
  ...
  if (source.concept === "NonoperatingIncomeExpense") {
    ...
    return /\bnon[-\s]?operating\b.*\b(income|expense|gain|loss|net|total)\b|\b(total|net)\b.*\bnon[-\s]?operating\b/.test(text);
  }
...
function resolveSellingGeneralAdministrativeExpense(period: string, ctx: ResolveContext): ResolvedValue {
  ...
  const facts = ctx.duration.get(period);
  const componentConceptSources = uniqueFactSources(
  ...
  const semanticSources = withAcceptedModelRowClassifications(
    period,
    ctx,
    uniquePrimaryIncomeStatementLineSources(period, ctx, [...componentConceptSources, ...primaryStatementSemanticSources]),
    "SG&A"
  );
  if (semanticSources.length) {
    return {
      value: -semanticSources.reduce((total, source) => total + Math.abs(source.value), 0),
      sources: semanticSources,
      note:
        "Grouped from EDGAR selling, marketing, advertising, promotional, and general/administrative operating expense rows because no broad SG&A concept was reported.",
      classification: semanticSources.length > 1 ? "grouped" : "direct"
    };
  }
...
function resolveBeginningRevolverBalance(_period: string, _ctx: ResolveContext): ResolvedValue {
  return beginningBalanceUnavailable("revolver");
}

function resolveBeginningRetainedEarningsBalance(_period: string, _ctx: ResolveContext): ResolvedValue {
  return beginningBalanceUnavailable("retained-earnings");
}

function resolveBeginningAociBalance(_period: string, _ctx: ResolveContext): ResolvedValue {
...
function genericDebtCurrentBelongsInDebtInclCurrentRow(source: FactSource, ctx: ResolveContext) {
  if (source.concept !== "DebtCurrent") return false;
  if (!ctx.template?.hasDebtInclCurrentPortionRow || ctx.template.hasCurrentDebtRow === true || ctx.template.hasCurrentDebtMaturitiesRow === true) return false;
  return !sourceLooksLikeShortTermBorrowing(source);
}
...
function resolveDirectOtherCurrentLiabilities(period: string, ctx: ResolveContext): ResolvedValue {
  ...
  const directSources = direct ? withAcceptedOtherCurrentLiabilityClassifications(period, ctx, direct.sources) : [];
  ...
  if (directSources.length && primaryOtherCurrent.value !== null) {
    const primaryHasBroadBucket = primaryOtherCurrent.sources.some((source) => BROAD_OTHER_CURRENT_LIABILITY_CONCEPTS.includes(source.concept));
    if (!primaryHasBroadBucket) {
      const sources = uniqueFactSources([...primaryOtherCurrent.sources, ...directSources]);
      candidates.push({
        value: sources.reduce((total, source) => total + source.value, 0),
        sources,
        note: "Mapped to reported current liability lines and directly reported current liability concepts that do not have dedicated model rows.",
        classification: sources.length > 1 ? "grouped" : "direct"
      });
    }
  }
  if (directSources.length) {
    candidates.push({
      value: directSources.reduce((total, source) => total + source.value, 0),
      sources: directSources,
      note: "Mapped to directly reported current liability concepts that do not have dedicated model rows.",
      classification: directSources.length > 1 ? "grouped" : "direct"
    });
  }
  if (primaryOtherCurrent.value !== null) candidates.push(primaryOtherCurrent);
  const selected = chooseOtherCurrentLiabilityCandidate(period, ctx, candidates);
  ...
  return direct
    ? { value: null, sources: [], note: "Current liability source lines were classified into dedicated rows outside Other Current Liabilities." }
    : { value: null, sources: [] };
}

function chooseOtherCurrentLiabilityCandidate(period: string, ctx: ResolveContext, candidates: ResolvedValue[]) {
  const valid = candidates.filter((candidate) => candidate.value !== null);
  ...
  const currentDebtModeledSeparately = ctx.template ? !currentDebtBelongsInAccruedLiabilities(ctx.template) || debtRowUsesCombinedCurrentDebt(period, ctx) : true;
  const currentDebt = currentDebtModeledSeparately ? resolveCurrentDebt(period, ctx) : zeroResolved(C.currentDebt[0]);
  if (currentLiabilities && accountsPayable.value !== null && accruedLiabilities.value !== null && currentDebt.value !== null) {
    const expected = currentLiabilities.value - accountsPayable.value - accruedLiabilities.value - currentDebt.value;
    return valid
      .slice()
      .sort((a, b) => Math.abs((a.value ?? 0) - expected) - Math.abs((b.value ?? 0) - expected))[0];
  }

  return valid[0];
}

function resolveDirectOrPrimaryAccruedLiabilities(period: string, ctx: ResolveContext): ResolvedValue {
  const primaryAccrued = resolvedFromPrimarySources(
    primaryAccruedLiabilitySources(period, ctx),
    "Mapped to reported accrued operating current liability lines from the primary consolidated balance sheet."
  );
...
function adjustedBroadAccruedLiabilities(period: string, ctx: ResolveContext, direct: FactSource): ResolvedValue {
  ...
  const currentDebtModeledSeparately = ctx.template ? !currentDebtBelongsInAccruedLiabilities(ctx.template) || debtRowUsesCombinedCurrentDebt(period, ctx) : true;
  if (currentDebtModeledSeparately) {
    const currentDebt = reportedCurrentDebt(period, ctx);
    if (currentDebt?.value) exclusions.push(currentDebt);
  }

  if (ctx.template?.hasOtherCurrentLiabilityRow) {
    const currentOperatingLeases = currentOperatingLeaseLiabilitiesForOtherCurrent(period, ctx);
    if (currentOperatingLeases?.value) exclusions.push(currentOperatingLeases);
  }

  const excludedValue = exclusions.reduce((total, item) => total + (item.value ?? 0), 0);
  if (!excludedValue) return { value: direct.value, sources: [direct] };

  const adjustedValue = direct.value - excludedValue;
  if (adjustedValue < -Math.max(1_000_000, Math.abs(direct.value) * 0.01)) return { value: direct.value, sources: [direct] };

  return {
    value: adjustedValue,
    sources: compactSources([direct, ...exclusions]),
    note:
      "Derived accrued liabilities from a broad reported accrued/current liability bucket less current debt and current operating lease liabilities that are modeled in separate rows.",
    classification: "grouped"
  };
}
...
function currentOperatingLeaseLiabilitiesForOtherCurrent(period: string, ctx: ResolveContext): ResolvedValue | null {
  const direct = sum(period, ctx.instant, CURRENT_OPERATING_LEASE_LIABILITY_CONCEPTS);
  if (!direct) return null;
  const sources = withAcceptedOtherCurrentLiabilityClassifications(period, ctx, direct.sources).filter(sourceLooksLikeCurrentLeaseLiability);
  if (!sources.length) return null;
  return {
    value: sources.reduce((total, source) => total + source.value, 0),
    sources,
    note: "Current operating lease liabilities are grouped into Other Current Liabilities when there is no dedicated current lease row.",
    classification: sources.length > 1 ? "grouped" : "direct"
  };
}

function withAcceptedOtherCurrentLiabilityClassifications(period: string, ctx: ResolveContext, sources: FactSource[]) {
  return sources.flatMap((source) => {
    if (sourceLooksLikeCurrentLeaseLiability(source) && !sourceLooksLikeCurrentDebtMaturity(source)) return [source];
    const classified = classifiedSourceForModelRow(period, ctx, source, "Other Current Liabilities");
    if (classified === null) return [];
    if (classified) return [classified];
    return [source];
  });
...
function resolveModeledNonCurrentLiabilitiesSubtotal(period: string, ctx: ResolveContext): ResolvedValue {
  ...
  if (direct) {
    return {
      value: direct.value,
      sources: [direct],
      note: "Mapped to EDGAR non-current liabilities. Current debt is not added to this subtotal because the SEC non-current-liabilities total is already authoritative."
    };
  }
...
function resolveTreasuryStockOnly(period: string, ctx: ResolveContext): ResolvedValue {
  ...
  if (treasury && treasury.value !== null) return treasury;
  const equity = resolveStockholdersEquity(period, ctx);
  const commonApic = resolveCommonStockAndApic(period, ctx);
...
function workbookInputBuffer(input: FillModelWorkbookInput["workbookBuffer"]) {
...
export async function fillModelWorkbook(input: FillModelWorkbookInput): Promise<FillModelWorkbookResult> {
  ...
  const timing = createFillTiming(query);
    ...
    if (!sheet) throw new FillModelServiceError(`Could not find a worksheet with historical income statement or balance sheet input rows in this workbook.`, 400);
    ...
    ensureFormulaDisplayCaches(workbook, formulaCacheColumns, unique([sheet.name, MODEL_SHEET, SEGMENT_SHEET]));
    refreshFinalIncomeStatementKeyMetrics(sheet, incomeStatementPeriods, incomeStatementColumns, ctx, auditRows);
    const partialBalanceSheetCheckResult = reconcilePartialBalanceSheetCheck(sheet, balanceSheetPeriods, balanceSheetColumns);
...
function expectedReportedLineItemCategory(fillRow: FillRow): ReportedLineItemCategory | null {
  ...
  if (
    fillRow.resolver === resolveSellingGeneralAdministrativeExpense ||
    hasConcept(C.sga) ||
    /sellinggeneraladministrative|sga|generaladministrative|sellingmarketing|salesmarketing|marketingexpense|advertising|promotion|corporateoverhead|administrativeexpense|salesexpense|sellingexpense/.test(label)
...
function reportedLineItemCategory(source: FactSource): ReportedLineItemCategory {
  ...
  if (
    ...
    /\bcurrent liabilit|\baccounts? payable\b|\baccrued\b|\bpharmacy claims?\b|\bhealth care costs? payable\b|\bother insurance liabilit.*\bcurrent\b|\bcurrent\b.*\bother insurance liabilit|\bcurrent portion\b.*\blease liabilit|\bdeferred (revenue|income)(?!\s+tax)\b|\bunearned revenue\b|\bcontract liabilit|\bcustomer advances?\b|\btaxes payable\b|\bshort[-\s]?term borrowings?\b|\bcommercial paper\b|\bcurrent portion\b.*\blong[-\s]?term debt\b/.test(text)
...
function reconcileFinalIncomeStatementConsistency(
  ...
  const results = [
    ...
    reconcileIncomeStatementFormulaRowsToEdgar(sheet, periods, columns, ctx),
    reconcilePreTaxIncomeFormulaRowsToEdgar(sheet, periods, columns, ctx),
    reconcileNetIncomeFormulaRowsToEdgar(sheet, periods, columns, ctx)
  ];
...
function llmCanUse(state: LlmMappingState, minRemainingMs = 1_000) {
  ...
  if (!state.budgetWarningAdded) {
    ...
    state.warnings.push("LLM mapping budget was exhausted; remaining rows used deterministic EDGAR classification and fail-closed validation.");
  }
  return false;
}
...
function llmWorkbenchIncomeAssignments(rows: PrimaryIncomeStatementAssignmentLedgerRow[]): LlmWorkbenchIncomeAssignment[] {
  return rows.map((row) => ({
    period: row.fiscalPeriod,
    sourceLineItemLabel: reviewText(row.sourceLineItemLabel, 500),
    sourceXbrlTag: row.sourceXbrlTag,
...
function automaticValidationRetryTrigger(errors: string[]) {
  ...
  const incomeStatementErrors = errors.filter(isRetriableIncomeStatementValidationError);
  if (balanceSheetErrors.length && incomeStatementErrors.length) {
    return "primary income-statement and balance-sheet assignment ledgers did not reconcile; mapped input rows are being rebuilt";
  }
  if (incomeStatementErrors.length) {
    return "income statement assignment ledger did not reconcile; primary income-statement rows are being rebuilt";
  }
...
function isRetriableIncomeStatementValidationError(error: string) {
  if (!/^Income Statement\b/i.test(error)) return false;
...
function repairBalanceSheetValidationFailure(options: WorkbookValidationRetryOptions, errors: string[]) {
  ...
  const postRefreshIncomeLedgerRebuild = rebuildPrimaryIncomeStatementRowsFromAssignmentLedger(options, { audit: false, targetKeys: incomeRepairTargets.assignmentKeys });
  warnings.push(...postRefreshIncomeLedgerRebuild.warnings);
  ...
  const postRefreshLedgerRebuild = rebuildPrimaryBalanceSheetRowsFromAssignmentLedger(options, { audit: false, targetKeys: repairTargets.assignmentKeys });
  ...
  if (options.isStandardModelSheet) {
    const checkResult = reconcileBalanceSheetCheck(options.sheet, options.balanceSheetPeriods, options.balanceSheetColumns);
    warnings.push(...checkResult.warnings);
    const partialCheckResult = reconcilePartialBalanceSheetCheck(options.sheet, options.balanceSheetPeriods, options.balanceSheetColumns);
  ...
  return {
    ...
    rebuiltRows: unique([
      ...
      ...postRefreshIncomeLedgerRebuild.rebuiltRows
    ]),
    diagnoses,
...
function diagnoseValidationFailures(errors: string[]) {
  ...
  add("income-statement classification", /income statement|gross profit|operating expense|pre-tax|tax bridge|net income|ebit|classification failure/i);
...
function balanceSheetValidationRepairTargets(errors: string[], sheet: ExcelJS.Worksheet) {
  ...
  for (const error of errors) {
    ...
    const likelyRowMatch = error.match(/^Balance Sheet\s+(?:[A-Z]+\d+\s+)?([^:]+):[\s\S]*?Likely row:\s+.+?\([A-Z]+(\d+)\)/i);
    if (likelyRowMatch) rowKeys.add(balanceSheetRepairRowKey(likelyRowMatch[1].trim(), Number(likelyRowMatch[2])));

    const assignmentMatch = error.match(/^Balance Sheet\s+[A-Z]+\d+\s+([^:]+):\s+model row "([^"]+)"/i);
    if (assignmentMatch) {
    ...
    const missingModelRowMatch = error.match(/^Balance Sheet\s+([^:]+):\s+assignment ledger mapped .* model row "([^"]+)"/i);
    if (missingModelRowMatch) assignmentKeys.add(primaryBalanceSheetAssignmentGroupKey(balanceSheetInstantLookupPeriod(missingModelRowMatch[1].trim()), missingModelRowMatch[2]));

    const nonCurrentLiabilitySubtotalMatch = error.match(/^Balance Sheet\s+[A-Z]+\d+\s+([^:]+):\s+total non-current liabilities .*does not equal disclosed component sum/i);
    if (nonCurrentLiabilitySubtotalMatch) {
      const debtRow = findBalanceSheetRow(sheet, ["LT Debt (Incl. Current Portion)", "Long Term Debt", "Long-Term Debt", "Senior Notes", "Borrowings", "Total Debt"]);
      if (debtRow) {
        const period = balanceSheetInstantLookupPeriod(nonCurrentLiabilitySubtotalMatch[1].trim());
        rowKeys.add(balanceSheetRepairRowKey(period, debtRow));
      }
    }

    const equitySubtotalMatch = error.match(/^Balance Sheet\s+[A-Z]+\d+\s+([^:]+):\s+total shareholders'? equity .*does not equal disclosed component sum/i);
    if (equitySubtotalMatch) {
      const period = balanceSheetInstantLookupPeriod(equitySubtotalMatch[1].trim());
      [
        ...
        const rowNumber = findBalanceSheetRow(sheet, labels);
        if (rowNumber) rowKeys.add(balanceSheetRepairRowKey(period, rowNumber));
      });
    }
  }
...
function incomeStatementValidationRepairTargets(errors: string[]) {
  const assignmentKeys = new Set<string>();
  for (const error of errors) {
    const assignmentMatch = error.match(/^Income Statement\s+[A-Z]+\d+\s+([^:]+):\s+model row "([^"]+)"/i);
    if (assignmentMatch) assignmentKeys.add(primaryIncomeStatementAssignmentGroupKey(assignmentMatch[1].trim(), assignmentMatch[2]));
...
function strictPrimaryBalanceSheetRebuild(options: WorkbookValidationRetryOptions, config: { targetRowKeys?: Set<string> } = {}) {
  ...
  for (const fillRow of options.fillRows) {
    ...
    for (const { period, col } of periodPairs) {
      ...
      if (config.targetRowKeys?.size && !config.targetRowKeys.has(balanceSheetRepairRowKey(period, fillRow.row))) continue;
      ...
      auditRow.formulaPreserved = canWriteReportedFormula && !replaceReportedFormula;
      if (canWriteReportedFormula) {
        auditRow.formulaStatus = replaceReportedFormula
          ? "reported-period formula replaced with SEC filing actual after validation retry"
          : "formula cached result refreshed from SEC filing actual";
      }
...
function rebuildPrimaryBalanceSheetRowsFromAssignmentLedger(
  options: WorkbookValidationRetryOptions,
  config: { audit?: boolean; targetKeys?: Set<string> } = {}
) {
  ...
  const warnings: string[] = [];
  const rebuiltRows = new Set<string>();
  const ledgerRows = buildPrimaryBalanceSheetAssignmentLedgerRows(options.balanceSheetPeriods, options.ctx, options.fillRows).filter(
    (row) => row.assignedModelRow && row.assignmentStatus !== "explicitly_excluded_with_reason"
  );
  if (!ledgerRows.length) return { filledCells, commentsAdded, warnings, rebuiltRows: [] };
  if (config.targetKeys && !config.targetKeys.size) return { filledCells, commentsAdded, warnings, rebuiltRows: [] };

  const periodPairs = uniquePeriodColumnPairs(options.balanceSheetPeriods.map((period, index) => ({ period, col: options.balanceSheetColumns[index] })));
  const rowsByPeriodAndModelRow = new Map<string, PrimaryBalanceSheetAssignmentLedgerRow[]>();
  for (const row of ledgerRows) {
    const key = primaryBalanceSheetAssignmentGroupKey(row.fiscalPeriod, row.assignedModelRow);
    const group = rowsByPeriodAndModelRow.get(key) ?? [];
    group.push(row);
  ...
  for (const { period, col } of periodPairs) {
    if (!col) continue;
    const lookupPeriod = balanceSheetInstantLookupPeriod(period);
    const modelRows = unique(ledgerRows.filter((row) => row.fiscalPeriod === lookupPeriod).map((row) => row.assignedModelRow).filter(Boolean));
    for (const modelRow of modelRows) {
      const targetKey = primaryBalanceSheetAssignmentGroupKey(lookupPeriod, modelRow);
      if (config.targetKeys && !config.targetKeys.has(targetKey)) continue;
      const assigned = rowsByPeriodAndModelRow.get(primaryBalanceSheetAssignmentGroupKey(lookupPeriod, modelRow)) ?? [];
      if (!assigned.length) continue;
      const rowNumber = findBalanceSheetLedgerModelRow(options.sheet, modelRow, options.fillRows);
      if (!rowNumber) {
        warnings.push(`Automatic balance-sheet repair could not find model row "${modelRow}" for ${period}; assignment ledger value was not written.`);
        continue;
      }
      if (isProjectedBalanceSheetCell(options.sheet, rowNumber, col)) continue;
      ...
      if (!forceTargetedRepair && !canRewritePrimaryBalanceSheetAssignmentCell(options.sheet, rowNumber, col, cell)) {
        warnings.push(
          `Automatic balance-sheet repair left ${cell.address} ${period} unchanged because the primary assignment ledger target "${modelRow}" is not a writable reported-period input cell.`
        );
        continue;
      }
      ...
      evaluator.clear();
      filledCells += 1;
      rebuiltRows.add(rowLabel(options.sheet, rowNumber) || modelRow);
  ...
  if (filledCells) warnings.push(`Automatic balance-sheet repair wrote ${filledCells} historical cell(s) from the primary balance-sheet assignment ledger.`);
  return { filledCells, commentsAdded, warnings, rebuiltRows: Array.from(rebuiltRows) };
}

function rebuildPrimaryIncomeStatementRowsFromAssignmentLedger(
  options: WorkbookValidationRetryOptions,
  config: { audit?: boolean; targetKeys?: Set<string> } = {}
) {
  const audit = config.audit !== false;
  ...
  const warnings: string[] = [];
  const rebuiltRows = new Set<string>();
  const ledgerRows = buildPrimaryIncomeStatementAssignmentLedgerRows(options.incomeStatementPeriods, options.ctx, options.fillRows).filter(
    (row) => row.assignedModelRow && incomeStatementLedgerModelRowIsComparable(row.assignedModelRow) && row.assignmentStatus !== "explicitly_excluded_with_reason" && row.assignmentStatus !== "subtotal_or_total_excluded"
  );
  if (!ledgerRows.length) return { filledCells, commentsAdded, warnings, rebuiltRows: [] };
  if (config.targetKeys && !config.targetKeys.size) return { filledCells, commentsAdded, warnings, rebuiltRows: [] };

  const periodPairs = uniquePeriodColumnPairs(options.incomeStatementPeriods.map((period, index) => ({ period, col: options.incomeStatementColumns[index] })));
  const rowsByPeriodAndModelRow = new Map<string, PrimaryIncomeStatementAssignmentLedgerRow[]>();
  for (const row of ledgerRows) {
    const key = primaryIncomeStatementAssignmentGroupKey(row.fiscalPeriod, row.assignedModelRow);
    const group = rowsByPeriodAndModelRow.get(key) ?? [];
    group.push(row);
    rowsByPeriodAndModelRow.set(key, group);
  }
  ...
  for (const { period, col } of periodPairs) {
    ...
    for (const modelRow of modelRows) {
      const targetKey = primaryIncomeStatementAssignmentGroupKey(period, modelRow);
      if (config.targetKeys && !config.targetKeys.has(targetKey)) continue;
      const assigned = rowsByPeriodAndModelRow.get(targetKey) ?? [];
      if (!assigned.length) continue;
      const rowNumber = findIncomeStatementLedgerModelRow(options.sheet, modelRow, options.fillRows);
      if (!rowNumber) {
        warnings.push(`Automatic income-statement repair could not find model row "${modelRow}" for ${period}; assignment ledger value was not written.`);
        continue;
      }
      ...
      if (actual !== null && statementMetricTies(actual, expected)) continue;
      const forceTargetedRepair = Boolean(config.targetKeys?.has(targetKey));
      if (!forceTargetedRepair && !canRewritePrimaryIncomeStatementAssignmentCell(options.sheet, rowNumber, col, cell)) {
        warnings.push(
          `Automatic income-statement repair left ${cell.address} ${period} unchanged because the primary assignment ledger target "${modelRow}" is not a writable reported-period input cell.`
        );
        continue;
      }
      ...
      filledCells += 1;
      rebuiltRows.add(rowLabel(options.sheet, rowNumber) || modelRow);
      const note = primaryIncomeStatementAssignmentAuditNote(rowLabel(options.sheet, rowNumber) || modelRow, assigned);
      if (addComment(cell, note)) commentsAdded += 1;
      if (audit) {
        options.auditRows.push(primaryIncomeStatementAssignmentAuditRow(options.sheet, cell, rowLabel(options.sheet, rowNumber) || modelRow, period, expected, assigned, Boolean(formulaBefore), note));
      }
    }
  }
...
function primaryBalanceSheetAssignmentGroupKey(period: string, modelRow: string) {
  return `${period}::${normalize(modelRow)}`;
}

function canRewritePrimaryIncomeStatementAssignmentCell(sheet: ExcelJS.Worksheet, rowNumber: number, col: number, cell: ExcelJS.Cell) {
  const label = rowLabel(sheet, rowNumber);
  if (!label || isProtectedCheckRowLabel(label)) return false;
  if (!findIncomeStatementMetricRow(sheet, [label])) return false;
  if (isProtectedFormulaOrCheckCell(cell)) return false;
  if (formulaForCell(cell)) return Boolean(reportedPeriodColumnsBySheet.get(sheet)?.has(col));
  return isHardcodedFinancialInput(cell);
}

function primaryIncomeStatementAssignmentAuditNote(modelRow: string, assigned: PrimaryIncomeStatementAssignmentLedgerRow[]) {
  const labels = unique(assigned.map((row) => row.sourceLineItemLabel).filter(Boolean));
  return lineItemSentence(modelRow, labels, assigned.length > 1 ? "includes" : "maps");
}

function primaryIncomeStatementAssignmentAuditRow(
  sheet: ExcelJS.Worksheet,
  cell: ExcelJS.Cell,
  modelRow: string,
  period: string,
  ...
  return {
    sheetName: sheet.name,
    cell: cell.address,
    modelRowLabel: modelRow,
    section: "Income Statement",
    period,
    valueWritten,
    mappingType: assigned.length > 1 || assigned.some((row) => row.assignmentStatus === "grouped_into_model_row") ? "grouped" : "direct",
    conceptsUsed: assigned.map((row) => `${row.sourceXbrlTag}=${roundModelValue(row.modelAmount / 1_000_000)}mm`).join("; "),
    secLabels: unique(assigned.map((row) => row.sourceLineItemLabel).filter(Boolean)).join("; "),
    sourceStatement: "income",
    accession: unique(assigned.map((row) => row.sourceFilingAccession).filter(Boolean)).join("; "),
    sourceUrl: "",
    cellWritable: true,
    formulaPreserved: false,
    formulaStatus: replacedFormula
      ? "reported-period formula replaced with primary SEC income-statement assignment"
      : "historical input refreshed from primary SEC income-statement assignment",
    writeBlockedReason: "",
    signConvention: "normalized to model income-statement sign convention",
    confidence: assigned.some((row) => row.assignmentStatus === "grouped_into_model_row") ? "medium" : "high",
    validationStatus: "OK!",
    notes: note,
    finalSourceLineItems: unique(assigned.map((row) => row.sourceLineItemLabel).filter(Boolean)).join("; "),
    finalSourceSections: unique(assigned.map((row) => row.sourceSection).filter(Boolean)).join("; "),
    finalRecommendedModelRows: modelRow,
    classificationReasons: unique(assigned.map((row) => row.classificationReason).filter(Boolean)).join("; "),
    llmClassificationUsed: assigned.some((row) => row.llmUsed),
    mappingPassedValidation: assigned.every((row) => row.validationStatus === "OK!")
  };
}

function canRewritePrimaryBalanceSheetAssignmentCell(sheet: ExcelJS.Worksheet, rowNumber: number, col: number, cell: ExcelJS.Cell) {
  if (isProjectedBalanceSheetCell(sheet, rowNumber, col)) return false;
...
function primaryBalanceSheetAssignmentAuditNote(modelRow: string, assigned: PrimaryBalanceSheetAssignmentLedgerRow[]) {
  const labels = unique(assigned.map((row) => row.sourceLineItemLabel).filter(Boolean));
  return lineItemSentence(modelRow, labels, assigned.length > 1 ? "includes" : "maps");
}

function primaryBalanceSheetAssignmentAuditRow(
  sheet: ExcelJS.Worksheet,
  cell: ExcelJS.Cell,
  modelRow: string,
  period: string,
  ...
  return {
    ...
    valueWritten,
    mappingType: assigned.length > 1 || assigned.some((row) => row.assignmentStatus === "grouped_into_model_row") ? "grouped" : "direct",
    conceptsUsed: assigned.map((row) => `${row.sourceXbrlTag}=${roundModelValue(row.amount / 1_000_000)}mm`).join("; "),
    secLabels: unique(assigned.map((row) => row.sourceLineItemLabel).filter(Boolean)).join("; "),
...
function validateWorkbookBeforeReturn(
  ...
  if (modelSheet) {
    ...
    if (fillRows.length) {
      errors.push(...validatePrimaryIncomeStatementAssignmentCoverage(modelSheet, incomeStatementPeriods, incomeStatementColumns, ctx, evaluator, warnings, fillRows));
...
function refreshFinalBalanceSheetKeyMetrics(sheet: ExcelJS.Worksheet, periods: string[], columns: number[], ctx: ResolveContext, auditRows: MappingAuditRow[]) {
  ...
  refreshBalanceSheetInputFromResolver(
    ...
    ctx,
    auditRows,
    ["Treasury & Preferred Stock", "Preferred Stock"],
...
function recordIncomeStatementBridgeMismatch(warnings: string[], message: string) {
...
function primaryAssignmentSemanticKey(
  period: string,
  statement: SecFilingStatementStructure,
  row: SecFilingStatementStructure["rows"][number],
  source: FactSource,
...
function assignPrimaryBalanceSheetLineItem(
  ...
  const chooseCurrentInvestment = (): PrimaryBalanceSheetAssignment => {
  ...
  if (sourceLooksLikeNonTradeReceivable(source)) {
    return chooseOther(
      "Prepaid & Other Current Assets",
      ["Prepaid and Other Current Assets", "Other Current Assets"],
      "Non-trade, vendor, tax, and other receivables are grouped into prepaid and other current assets when the template has no dedicated non-trade receivables row."
    );
  }
  ...
  if (sourceLooksLikeCurrentInvestment(source)) {
    return chooseCurrentInvestment();
  }
  ...
  if (genericDebtCurrentBelongsInDebtInclCurrentRow(source, ctx)) {
    return choose(
      "LT Debt (Incl. Current Portion)",
      ["Long-Term Debt", "Long Term Debt", "Borrowings"],
      "Generic DebtCurrent maps to LT Debt including current portion when the template has no separate current debt row."
    );
  }
  if (genericCurrentDebtBelongsInCurrentMaturities(source, reportedStandaloneShortTermBorrowingSource(period, ctx), ctx)) {
    return choose(
      "LT Debt (Incl. Current Portion)",
      ["Long-Term Debt", "Long Term Debt", "Borrowings"],
      "Generic current debt maps to LT Debt including current portion when the template has no separate current debt row."
    );
  }
...
function balanceSheetModelTotalTiesEdgar(
  sheet: ExcelJS.Worksheet,
  labels: string[],
  col: number,
  source: FactSource,
  ...
  const rowNumber =
    ...
    balanceSheetSectionRows(sheet).find((candidateRow) => {
      const label = rowLabel(sheet, candidateRow);
      if (labels.some((candidate) => isTotalAssetsLabel(candidate))) return isTotalAssetsLabel(label);
      if (labels.some((candidate) => isTotalLiabilitiesAndEquityLabel(candidate))) return isTotalLiabilitiesAndEquityLabel(label);
      return false;
    }) ??
    null;
...
function assignPrimaryIncomeStatementLineItem(
  ...
  if (source.unit && !/usd/i.test(source.unit)) {
    return {
      modelRow: null,
      status: "explicitly_excluded_with_reason",
      reason: "Excluded non-USD primary income statement presentation row from dollar income-statement coverage."
    };
  }
  ...
  if (exclusionReason) {
    return {
      modelRow: null,
      status: "subtotal_or_total_excluded",
  ...
  const category = reportedLineItemCategory(source);
  if (section === "revenue") {
    return choose("Revenue", ["Revenues", "Total Revenue", "Total Revenues", "Sales", "Net Sales", "Net Revenue"], "Rows presented in the primary income-statement revenue block map to revenue or are excluded as components when a total revenue row is reported.");
  }
  ...
  if (category === "cost_of_revenue") {
    return choose(
      ...
      "Cost of revenue, sales, goods, services, and direct operating cost rows map to COGS / Cost of Goods Sold."
    );
  }
  if (category === "research_and_development") {
    return choose("R&D", ["Research & Development (R&D)", "Research and Development"], "Research, product development, engineering, and technology development operating expenses map to R&D.");
  }
  if (category === "selling_general_administrative") {
    return choose(
      "SG&A",
      ["Selling, General & Administration (SG&A)", "Selling General & Administrative", "Sales and Marketing", "Selling and Marketing", "General and Administrative"],
      "Selling, marketing, advertising, promotional, and administrative operating expenses map to SG&A."
    );
  }
  ...
  if (goodwillImpairmentScore(source) >= 9 && primaryIncomeStatementRowIsBelowOperatingBeforePreTaxForSourceRow(row)) {
    return choose("Goodwill Impairment", ["Impairment of Goodwill", "Goodwill and Intangible Asset Impairment"], "Standalone goodwill impairment rows map to goodwill impairment.");
  }
  if (category === "interest_income") {
    return choose("Interest Income", [], "Standalone interest income rows map to interest income.");
  }
  if (category === "interest_expense") {
    return choose("Interest Expense", ["Interest (Expense)"], "Standalone interest expense rows map to interest expense.");
  }
  if (category === "income_tax") {
    return choose(
      "Income Tax Benefit / Expense",
      ["Income Tax Expense", "Income Tax Provision (Expense)", "Income Tax"],
  ...
  return {
    modelRow: null,
    status: "explicitly_excluded_with_reason",
    reason: "Could not determine a reusable income-statement model row from SEC concept, label, and statement section."
  };
}
...
function classifiedPrimaryIncomeStatementAssignment(
  ...
  const classification = lineItemClassificationForSource(period, ctx, source);
  if (classification && !classification.llm_used) return null;
...
function incomeStatementAssignmentAmount(source: FactSource, modelRow: string | null) {
  ...
  if (modelRowsMatch(modelRow, "COGS / Cost of Goods Sold")) return expenseAsModelReduction(source.value);
  if (modelRowsMatch(modelRow, "SG&A")) return expenseAsModelReduction(source.value);
  if (modelRowsMatch(modelRow, "R&D")) return expenseAsModelReduction(source.value);
  if (modelRowsMatch(modelRow, "D&A")) return expenseAsModelReduction(source.value);
  if (modelRowsMatch(modelRow, "Interest Expense")) return expenseAsModelReduction(source.value);
  if (modelRowsMatch(modelRow, "Goodwill Impairment")) return expenseAsModelReduction(source.value);
  if (modelRowsMatch(modelRow, "Income Tax Benefit / Expense")) return -source.value;
  if (modelRowsMatch(modelRow, "Other Operating Income / Expense")) return otherOperatingLineValue(source);
  if (modelRowsMatch(modelRow, "Other Non-Operating Income / Expense")) return otherNonOperatingValue(source);
  return source.value;
}

function incomeStatementAssignmentStatusForModelRow(modelRow: string, source: FactSource): IncomeStatementAssignmentStatus {
  if (
    modelRowsMatch(modelRow, "Other Operating Income / Expense") ||
    modelRowsMatch(modelRow, "Other Non-Operating Income / Expense") ||
    (modelRowsMatch(modelRow, "Revenue") && isRevenueComponentSource(source)) ||
    (modelRowsMatch(modelRow, "COGS / Cost of Goods Sold") && !C.cogs.includes(source.concept)) ||
    (modelRowsMatch(modelRow, "SG&A") && !C.sga.includes(source.concept))
  ) {
    return "grouped_into_model_row";
  }
  return "mapped_to_model_row";
...
function incomeStatementAssignmentModelRowIsAssignable(modelRow: string | null) {
  ...
  return [
    ...
    "Other Non-Operating Income / Expense",
    "Income Tax Benefit / Expense"
  ].some((candidate) => modelRowsMatch(modelRow, candidate));
}

function incomeStatementLedgerModelRowIsComparable(modelRow: string | null) {
  return incomeStatementAssignmentModelRowIsAssignable(modelRow);
}

function primaryIncomeStatementAssignmentExclusionReason(
  period: string,
  ctx: ResolveContext,
  source: FactSource,
  row: PrimaryIncomeStatementRow,
  ...
  if (isComprehensiveIncomeSource(source)) {
    return "Excluded other comprehensive income / AOCI reclassification row because it is not a primary income-statement operating, non-operating, tax, or net-income component.";
  }
  if (primaryIncomeStatementRevenueComponentCoveredByTotal(period, ctx, source, row, section)) {
    return "Excluded revenue component/detail row because the same primary income statement reports an additive total revenue line that is assigned to the model.";
  }
  if (primaryIncomeStatementCostComponentCoveredByTotal(period, ctx, source, row)) {
    return "Excluded cost component/detail row because the same primary income statement reports an additive total cost of revenue line that is assigned to the model.";
  }
  if (primaryIncomeStatementTaxComponentCoveredByTotal(period, ctx, source, row)) {
    return "Excluded income-tax component/detail row because the same primary income statement reports a total income-tax provision line that is assigned to the model.";
  }
  if (sourceLooksLikePerShareOrShareCountIncomeStatementLine(source)) {
    return "Excluded per-share, share-count, or earnings-per-share presentation row because it is not a dollar income-statement amount.";
  }
  const category = reportedLineItemCategory(source);
  if (category === "operating_income" || category === "pretax_income" || category === "net_income") {
    return "Excluded income-statement subtotal/total row because the model calculates this row from assigned components.";
  }
  if (incomeStatementSourceIsSubtotalOrTotalToExclude(source, section)) {
    return "Excluded income-statement subtotal/total row because it should not be double-counted with its components.";
  }
  return "";
}

function primaryIncomeStatementRevenueComponentCoveredByTotal(
  period: string,
  ctx: ResolveContext,
  source: FactSource,
  row: PrimaryIncomeStatementRow,
  ...
  if (reportedLineItemCategory(source) !== "revenue" && section !== "revenue") return false;
  if (primaryIncomeStatementRowLooksLikeTotalRevenue(row)) return false;
  if (!isRevenueComponentSource(source) && section !== "revenue" && !row.dimensions.length && !row.parentSubtotal) return false;
  return primaryIncomeStatementRowsForPeriod(period, ctx).some(({ row: candidateRow }) => {
    if (candidateRow === row) return false;
    const candidate = factSourceFromStatementRow(candidateRow, period);
    if (!candidate) return false;
    if (isRevenueComponentSource(candidate) || candidateRow.dimensions.length) return false;
    return primaryIncomeStatementRowLooksLikeTotalRevenue(candidateRow);
  });
}

function primaryIncomeStatementCostComponentCoveredByTotal(period: string, ctx: ResolveContext, source: FactSource, row: PrimaryIncomeStatementRow) {
  if (reportedLineItemCategory(source) !== "cost_of_revenue") return false;
  if (!row.dimensions.length && !row.parentSubtotal && (C.cogs.includes(source.concept) || sourceTextMatches(source, /\b(total )?cost of (revenue|sales|goods|services|products)\b/))) return false;
  return primaryIncomeStatementRowsForPeriod(period, ctx).some(({ row: candidateRow }) => {
    if (candidateRow === row) return false;
    const candidate = factSourceFromStatementRow(candidateRow, period);
    if (!candidate || reportedLineItemCategory(candidate) !== "cost_of_revenue") return false;
    if (candidateRow.dimensions.length) return false;
    return C.cogs.includes(candidate.concept) || sourceTextMatches(candidate, /\b(total )?cost of (revenue|sales|goods|services|products)\b/);
  });
}

function primaryIncomeStatementTaxComponentCoveredByTotal(period: string, ctx: ResolveContext, source: FactSource, row: PrimaryIncomeStatementRow) {
  if (reportedLineItemCategory(source) !== "income_tax") return false;
  if (sourceLooksLikeIncomeTaxTotal(source) && !sourceLooksLikeIncomeTaxDetail(source)) return false;
  if (!sourceLooksLikeIncomeTaxDetail(source) && !row.dimensions.length && !row.parentSubtotal) return false;
  return primaryIncomeStatementRowsForPeriod(period, ctx).some(({ row: candidateRow }) => {
    if (candidateRow === row) return false;
    const candidate = factSourceFromStatementRow(candidateRow, period);
    return Boolean(candidate && reportedLineItemCategory(candidate) === "income_tax" && sourceLooksLikeIncomeTaxTotal(candidate));
  });
}

function sourceLooksLikeIncomeTaxTotal(source: FactSource) {
  if (source.concept === "IncomeTaxExpenseBenefit" && !sourceLooksLikeIncomeTaxDetail(source)) return true;
  return sourceTextMatches(source, /\b(provision for|benefit from|expense for)?\s*income taxes?\b|\bincome tax (?:expense|benefit|provision)\b/);
}

function sourceLooksLikeIncomeTaxDetail(source: FactSource) {
  return sourceTextMatches(
    source,
    /\bshare[-\s]?based compensation\b|\bstock[-\s]?based compensation\b|\bexcess tax benefits?\b|\brelated to\b|\bcurrent\b.*\btax\b|\bdeferred\b.*\btax\b|\btax credits?\b|\bforeign\b.*\btax\b|\bfederal\b.*\btax\b|\bstate\b.*\btax\b/
  );
}
...
function incomeStatementSourceIsSubtotalOrTotalToExclude(source: FactSource, section: FinancialStatementSection) {
  if (section === "revenue" && reportedLineItemCategory(source) === "revenue") return false;
  if (section === "below operating income" && isBelowOperatingBridgeComponentSource(source)) return false;
  const text = sourceSearchText(source);
  const compact = sourceCompactText(source);
...
function isBelowOperatingBridgeComponentSource(source: FactSource) {
  if (isCombinedBelowOperatingSummaryLine(source)) return false;
  if (isExplicitInterestIncomeSource(source) || isExplicitInterestExpenseSource(source)) return true;
  if (GOODWILL_IMPAIRMENT_CONCEPTS.includes(source.concept) || goodwillImpairmentScore(source) >= 9) return true;
  if (OTHER_NON_OPERATING_CONCEPTS.includes(source.concept) || otherNonOperatingScore(source) >= 5) return true;
  return false;
}

function primaryIncomeStatementRowIsBelowOperatingBeforePreTaxForSourceRow(row: PrimaryIncomeStatementRow) {
  const syntheticStatement: SecFilingStatementStructure = {
    accession: "",
    filingDate: "",
    form: "",
    reportingPeriod: "",
  ...
  return primaryIncomeStatementRowIsBelowOperatingBeforePreTax(syntheticStatement, row) || isBelowOperatingLineLabel(row.rowLabel);
}

function primaryIncomeStatementAssignmentRowKey(statement: SecFilingStatementStructure, row: PrimaryIncomeStatementRow, period: string) {
  return statementLineItemSourceRowKey(statement, "income_statement", period, row);
}

function primaryIncomeStatementAssignmentGroupKey(period: string, modelRow: string) {
  return `${period}::${normalize(modelRow)}`;
}

function duplicateIncomeStatementAssignmentKeys(rows: PrimaryIncomeStatementAssignmentLedgerRow[]) {
  const seen = new Set<string>();
...
function findIncomeStatementLedgerModelRow(sheet: ExcelJS.Worksheet, modelRow: string, fillRows: FillRow[]) {
  const fillRow = fillRows.find((row) => row.statement === "income" && row.kind === "duration" && modelRowsMatch(row.label, modelRow));
  if (fillRow) return fillRow.row;
...
function validatePrimaryBalanceSheetAssignmentCoverage(
  ...
  for (const { period, col } of periodPairs) {
    ...
    byModelRow.forEach((assigned, modelRow) => {
      ...
      if (!rowNumber) {
        const message = `Balance Sheet ${period}: assignment ledger mapped ${assigned.length} source line(s) to missing model row "${modelRow}".`;
        errors.push(message);
        return;
      }
      if (isProjectedBalanceSheetCell(sheet, rowNumber, col)) return;
      ...
      if (!statementMetricTies(actual, expected)) {
        ...
        if (modelRowsMatch(modelRow, "LT Debt (Incl. Current Portion)")) {
          const resolvedDebt = resolveLongTermDebtInclCurrentPortion(lookupPeriod, ctx);
          const resolvedDebtValue = resolvedDebt.value === null ? null : resolvedDebt.value / 1_000_000;
          if (resolvedDebtValue !== null && statementMetricTies(actual, resolvedDebtValue)) {
            warnings.unshift(
              `${message} The model row ties the EDGAR debt resolver, which may derive current maturities from EDGAR facts outside the primary assignment row.`
            );
            return;
          }
        }
...
function validatePrimaryIncomeStatementAssignmentCoverage(
  sheet: ExcelJS.Worksheet,
  periods: string[],
  columns: number[],
  ctx: ResolveContext,
  ...
  for (const { period, col } of periodPairs) {
    ...
    const unsupportedRows = rows.filter(
      (row) =>
        ...
        Math.abs(row.sourceAmount) > 0.5 &&
        !allowedPrimaryIncomeStatementAssignmentExclusion(row)
    );
    unsupportedRows.forEach((row) => {
    ...
    const byModelRow = new Map<string, PrimaryIncomeStatementAssignmentLedgerRow[]>();
    const assignedRows = rows.filter((row) => row.assignedModelRow && incomeStatementLedgerModelRowIsComparable(row.assignedModelRow));
    for (const row of assignedRows) {
      const group = byModelRow.get(row.assignedModelRow) ?? [];
      group.push(row);
      byModelRow.set(row.assignedModelRow, group);
    ...
    byModelRow.forEach((assigned, modelRow) => {
      ...
      if (!rowNumber) {
        ...
        warnings.unshift(`${message} Assignment ledger row-level coverage was recorded for review but did not block workbook output.`);
        return;
      }
      const expected = assigned.reduce((total, row) => total + row.modelAmount, 0) / 1_000_000;
      const cell = sheet.getCell(rowNumber, col);
      ...
      const actual = statementMetricCellValue(cell, evaluator, expected);
      if (actual === null) {
        recordIncomeStatementAssignmentValidationIssue(
          errors,
          warnings,
          `Income Statement ${cell.address} ${period}: could not evaluate assigned model row "${modelRow}" against primary statement assignment ledger.`,
          protectedFormula
        );
        return;
      }
...
function recordIncomeStatementAssignmentValidationIssue(
  errors: string[],
  warnings: string[],
  message: string,
  protectedFormula: boolean
...
function latestPrimaryIncomeStatementAssignmentPeriod(periods: string[], ctx: ResolveContext) {
  const lookupPeriods = unique(periods.filter(isSupportedPeriodKey));
  const periodsWithPrimaryRows = lookupPeriods.filter((period) => primaryIncomeStatementRowsForPeriod(period, ctx).length > 0);
...
function findBalanceSheetLedgerModelRow(sheet: ExcelJS.Worksheet, modelRow: string, fillRows: FillRow[]) {
  const primaryRow = findBalanceSheetRow(sheet, [modelRow]);
...
function validateBalanceSheetClassificationCompleteness(
  ...
  const criticalChecks: Array<{
    ...
    { labels: ["Treasury Stock"], metricName: "treasury stock", resolver: resolveTreasuryStockOnly },
    { labels: ["Treasury & Preferred Stock", "Preferred Stock"], metricName: "treasury/preferred stock", resolver: resolveTreasuryAndPreferredStock },
  ...
  for (const year of years) {
    ...
    for (const rowNumber of sectionRows) {
      ...
      if (!statementMetricTies(fourthQuarter, annual)) {
        ...
        const annualResolved = resolver ? resolver(balanceSheetInstantLookupPeriod(`FY${year}`), ctx) : null;
        const annualExpected = annualResolved?.value !== null && annualResolved?.value !== undefined ? annualResolved.value / 1_000_000 : null;
        if (annualExpected !== null && statementMetricTies(annual, annualExpected)) continue;
        errors.push(
...
function validateBalanceSheetSubtotalEqualsComponents(
  ...
  if (metricName === "total non-current liabilities" && balanceSheetCurrentLiabilitiesSubtotalExcludesDebt(sheet)) {
    effectiveComponentLabelGroups.push(["Short Term Borrowings", "Short-term Borrowings", "Short-Term Debt", "Short Term Debt", "Revolver"]);
  }
  let totalLiabilitiesCurrentSubtotalExcludesDebt = false;
  if (metricName === "total liabilities") {
    ...
    totalLiabilitiesCurrentSubtotalExcludesDebt = currentLiabilitiesSubtotalExcludesDebtLabel(currentLiabilitiesLabel);
    if (totalLiabilitiesCurrentSubtotalExcludesDebt && !balanceSheetDebtRowIncludesCurrentPortion(sheet)) {
      effectiveComponentLabelGroups.push(
        ["Current Debt"],
        ["Current Portion of Long-Term Debt", "Current Maturities of Long-Term Debt", "Debt Due Within One Year"],
        ["Short Term Borrowings", "Short-term Borrowings", "Short-Term Debt", "Short Term Debt", "Revolver"]
  ...
  periods.forEach((period, index) => {
    ...
    if (metricName === "total liabilities" && totalLiabilitiesCurrentSubtotalExcludesDebt && !statementMetricTies(total, baseExpected)) {
      const currentDebtRows = [
        ["Current Debt"],
        ["Current Portion of Long-Term Debt", "Current Maturities of Long-Term Debt", "Debt Due Within One Year"],
        ["Short Term Borrowings", "Short-term Borrowings", "Short-Term Debt", "Short Term Debt", "Revolver"]
      ]
      ...
      if (currentDebtRows.length && currentDebtValues.every((value) => value !== null)) {
        ...
        if (statementMetricTies(total, expectedWithCurrentDebt)) expected = expectedWithCurrentDebt;
      }
    }
    if (!statementMetricTies(total, expected)) {
      ...
      if (metricName === "total non-current liabilities") {
        warnings.unshift(`${message} The SEC total was preserved; component presentation differs because current-debt and non-current-liability rows are mapped separately in the template.`);
        return;
      }
      if (metricName === "total current liabilities" && currentLiabilitiesSubtotalExcludesDebtLabel(totalLabel)) {
...
function balanceSheetCurrentLiabilitiesSubtotalExcludesDebt(sheet: ExcelJS.Worksheet) {
  const rowNumber = findBalanceSheetRow(sheet, [
    "Total Current Liabilities",
    "Total Current Liabilities (Excl. Debt)",
    "Total Current Liabilities Excl. Debt"
  ]);
  return rowNumber ? currentLiabilitiesSubtotalExcludesDebtLabel(rowLabel(sheet, rowNumber)) : false;
}

function balanceSheetDebtRowIncludesCurrentPortion(sheet: ExcelJS.Worksheet) {
  const debtRow = findBalanceSheetRow(sheet, ["LT Debt (Incl. Current Portion)", "Long Term Debt", "Long-Term Debt", "Senior Notes", "Borrowings", "Total Debt"]);
  return debtRow ? debtRowLabelIncludesCurrentPortion(rowLabel(sheet, debtRow)) : false;
}

function debtRowLabelIncludesCurrentPortion(label: string) {
  const normalized = normalizedRowLabel(label);
  return /(?:incl|including|includes).*currentportion|currentportion.*(?:incl|including|included)|totaldebt/.test(normalized);
}
...
function balanceSheetDiagnosticResolverForLabel(label: string): ((period: string, ctx: ResolveContext) => ResolvedValue) | null {
  ...
  if (/^treasurystock$/.test(normalizedLabel)) return resolveTreasuryStockOnly;
  if (/^(treasurypreferredstock|preferredstock)$/.test(normalizedLabel)) return resolveTreasuryAndPreferredStock;
...
function snapshotWorkbook(workbook: ExcelJS.Workbook, sheetNames: string[], firstHistoricalCol: number): WorkbookSnapshot {
  ...
  for (const sheetName of sheetNames) {
    const sheet = workbook.getWorksheet(sheetName);
    if (!sheet) continue;
    for (const rowNumber of workbookSnapshotRowNumbers(sheet)) {
...
function workbookSnapshotRowNumbers(sheet: ExcelJS.Worksheet) {
  const rows = new Set<number>();
  sheet.eachRow({ includeEmpty: false }, (_row, rowNumber) => rows.add(rowNumber));
  for (const rowNumber of balanceSheetRows(sheet)) rows.add(rowNumber);
...
function protectedCellFingerprint(cell: ExcelJS.Cell) {
  ...
  return JSON.stringify({
    value: formula ? { formula } : serializableCellValue(cell.value),
    note: cell.note ?? null
  });
}
...
function addIncomeStatementAssignmentLedgerSheet(workbook: ExcelJS.Workbook, ledgerRows: PrimaryIncomeStatementAssignmentLedgerRow[]) {
  const existing = workbook.getWorksheet(INCOME_STATEMENT_ASSIGNMENT_LEDGER_SHEET);
  if (existing) workbook.removeWorksheet(existing.id);
  const sheet = workbook.addWorksheet(INCOME_STATEMENT_ASSIGNMENT_LEDGER_SHEET);
  sheet.columns = [
    { header: "fiscal period", key: "fiscalPeriod", width: 14 },
    { header: "source filing accession", key: "sourceFilingAccession", width: 26 },
    { header: "source statement", key: "sourceStatement", width: 40 },
